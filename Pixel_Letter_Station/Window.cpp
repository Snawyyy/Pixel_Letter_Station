			...
			case INK_LETTER_BUTTON_ID:
			{
				if (letterOpened == false)
				{
					...
					if (hBitmap != NULL) // Check if the bitmap handle is valid
					{
						...
						CreateUserWindow(hWnd, hInstance, 100, 100, LETTER_BOX_WIDTH + (SMALL_MARGIN * 2) + (BAR_MARGIN * 2) - 1 + (SMALL_MARGIN * 2), height - (MARGIN * 5.5) + WIN_BAR_SIZE + BAR_MARGIN + (SMALL_MARGIN * 3) + MARGIN + BUTTON_HEIGHT, hBitmap);
						letterOpened = true;
					}
					else
					{
